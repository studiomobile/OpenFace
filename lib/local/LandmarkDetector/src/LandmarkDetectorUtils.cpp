	...
	if(n >= 66)
	{
		...
		{		
			if(visibilities.at<int>(i))
			{
				...
				cv::circle(img, featurePoint, 0.35 * draw_multiplier, cv::Scalar(0, 0, 255), thickness, CV_AA, draw_shiftbits);
				cv::circle(img, featurePoint, 0.35 * draw_multiplier, cv::Scalar(255, 0, 0), thickness_2, CV_AA, draw_shiftbits);
